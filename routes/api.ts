...
export default async function (router: Router) {
  await router.post('/disk-scan', async () => {
    ...
    const worker = new Worker(new URL('../workers/disk-scan.ts', import.meta.url).href)
    ...
    return new Promise<Response>((resolve) => {
      ...
      worker.onerror = (e: ErrorEvent) => {
        clearTimeout(timeout)
        worker.terminate()
        resolve(Response.json({ success: false, error: e.message || 'Scan failed' }))
      }
  ...
  await router.post('/delete-path', async (req) => {
    ...
    if (!target) return Response.json({ success: false, error: 'No path provided' }, { status: 400 })
    const check = isPathSafe(target)
    if (!check.safe) return Response.json({ success: false, error: check.reason }, { status: 403 })
  ...
  await router.post('/clean-dir', async (req) => {
    ...
    return Response.json({ success: result.errors.length === 0, freedBytes: result.freedBytes, errors: result.errors.length ? result.errors : undefined })
  })

  await router.post('/kill-process', async (req) => {
    const { pid } = await req.json() as { pid: number }
    if (!pid) return Response.json({ success: false, error: 'No PID provided' }, { status: 400 })
  ...
  await router.post('/toggle-startup', async (req) => {
    ...
    if (!filepath) return Response.json({ success: false, error: 'No filepath provided' }, { status: 400 })
  ...
  await router.post('/dir-sizes', async (req) => {
    ...
    if (!paths || !Array.isArray(paths)) return Response.json({ success: false, error: 'No paths provided' }, { status: 400 })
  ...
  await router.post('/empty-trash', async () => {
    ...
    return Response.json({ success: result.success, freedBytes: result.freedBytes })
  })
}
